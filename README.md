...
除了内建的常用汉字表、常用韩文音节表、日本人名用汉字表，新增了 `datas/UsedChar_Custom.txt`，让使用者可以自行补充「以上清单都没收录、但希望转换后保留」的字——例如罕见国字、特定符号等。使用方式与其他 `UsedChar_*.txt` 相同：逐字一行，`#` 开头的整行视为注释。这份文件本身可选，不存在也不影响其他保留清单正常运作。

### 日本人名用汉字表

先前陆续发现被误删的字，有好几个都是日本人名用汉字（如辻、凪、栞、冴），意识到这是一整类有官方清单可查的字，不应该让使用者一个个手动补进自订清单。新增 `datas/UsedChar_JP_Jinmei.txt`：日本法务省「人名用汉字别表」（戸籍法施行规则别表第二）647字，跟常用汉字表（`UsedChar_JP.txt`，2136字）是并列但不同的官方清单——常用汉字表是义务教育与一般出版品用字，人名用汉字表专门补充「常用汉字以外、但可用于户籍登记姓名」的汉字。
...
原版工具在简转繁过程中，会把弯引号（`''`、`""`）替换成中文直角引号（`「」`、`『』`）——查证后确认这**不是** [OpenCC](https://github.com/BYVoid/OpenCC) 官方词库原本就有的规则（逐行核对 OpenCC 官方 `STCharacters.txt` 原始档案，只有汉字对照，没有任何标点符号规则），是这个工具的 `datas/Chars_st.txt`/`Chars_ts.txt` 额外加入的客製规则。本 fork 已移除这4条规则（双向对照表同步处理，保持行为一致），转换后引号符号本身维持原样，只转换文字内容。
