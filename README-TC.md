...
除了內建的常用漢字表、常用韓文音節表、日本人名用漢字表，新增了 `datas/UsedChar_Custom.txt`，讓使用者可以自行補充「以上清單都沒收錄、但希望轉換後保留」的字——例如罕見國字、特定符號等。使用方式與其他 `UsedChar_*.txt` 相同：逐字一行，`#` 開頭的整行視為註解。這份檔案本身可選，不存在也不影響其他保留清單正常運作。

### 日本人名用漢字表

先前陸續發現被誤刪的字，有好幾個都是日本人名用漢字（如辻、凪、栞、冴），意識到這是一整類有官方清單可查的字，不應該讓使用者一個個手動補進自訂清單。新增 `datas/UsedChar_JP_Jinmei.txt`：日本法務省「人名用漢字別表」（戸籍法施行規則別表第二）647字，跟常用漢字表（`UsedChar_JP.txt`，2136字）是並列但不同的官方清單——常用漢字表是義務教育與一般出版品用字，人名用漢字表專門補充「常用漢字以外、但可用於戶籍登記姓名」的漢字。
...
原版工具在簡轉繁過程中，會把彎引號（`''`、`""`）替換成中文直角引號（`「」`、`『』`）——查證後確認這**不是** [OpenCC](https://github.com/BYVoid/OpenCC) 官方詞庫原本就有的規則（逐行核對 OpenCC 官方 `STCharacters.txt` 原始檔案，只有漢字對照，沒有任何標點符號規則），是這個工具的 `datas/Chars_st.txt`/`Chars_ts.txt` 額外加入的客製規則。本 fork 已移除這4條規則（雙向對照表同步處理，保持行為一致），轉換後引號符號本身維持原樣，只轉換文字內容。
