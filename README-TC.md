...
除了內建的常用漢字表、常用韓文音節表，新增了 `datas/UsedChar_Custom.txt`，讓使用者可以自行補充「常用清單沒收錄、但希望轉換後保留」的字——例如日本人名用漢字、罕見國字等。使用方式與其他 `UsedChar_*.txt` 相同：逐字一行，`#` 開頭的整行視為註解。這份檔案本身可選，不存在也不影響其他保留清單正常運作。
